...
const TYPES_JS = /\b(string|number|boolean|object|symbol|bigint|any|unknown|never|void|Array|Object|Promise|Map|Set|Date|Math|JSON|console|document|window|localStorage)\b/g;
...
function highlightJs(code: string) {
  ...
  while (i < code.length) {
    ...
    // line comment
    let m = rest.match(/^\/\/[^\n]*/);
    if (m) { tokens.push({ type: "comment", text: m[0] }); i += m[0].length; continue; }
    // block comment
    m = rest.match(/^\/\*[\s\S]*?\*\//);
    if (m) { tokens.push({ type: "comment", text: m[0] }); i += m[0].length; continue; }
    // template literal
    if (ch === "`") {
      ...
      tokens.push({ type: "string", text: rest.slice(0, len) }); i += len; continue;
    }
    // string
    if (ch === '"' || ch === "'") {
      const re = new RegExp(`^${ch}(?:\\\\.|[^${ch}\\\\])*${ch}`);
      m = rest.match(re);
      if (m) { tokens.push({ type: "string", text: m[0] }); i += m[0].length; continue; }
    }
    // number
    m = rest.match(/^-?\d+(\.\d+)?n?\b/);
    if (m) { tokens.push({ type: "number", text: m[0] }); i += m[0].length; continue; }
    // identifier
    m = rest.match(/^[A-Za-z_$][\w$]*/);
    if (m) { tokens.push({ type: "ident", text: m[0] }); i += m[0].length; continue; }
  ...
  return tokens
    .map((t, idx) => {
      ...
      if (t.type === "comment") return `<span style="color:var(--color-code-comment)">${safe}</span>`;
      ...
      if (t.type === "ident") {
        if (new RegExp(KW.source).test(t.text)) return `<span style="color:var(--color-code-keyword);font-weight:600">${safe}</span>`;
        if (TYPES_JS.test(t.text)) { TYPES_JS.lastIndex = 0; return `<span style="color:var(--color-code-type)">${safe}</span>`; }
        const next = tokens[idx + 1];
        if (next && next.text === "(") return `<span style="color:var(--color-code-fn)">${safe}</span>`;
...
function highlightHtml(code: string) {
  ...
  return safe
    .replace(/(&lt;\/?)([a-zA-Z][\w-]*)/g, `$1<span style="color:var(--color-code-keyword);font-weight:600">$2</span>`)
    .replace(/([a-zA-Z-]+)=(&quot;[^&]*&quot;|"[^"]*")/g, `<span style="color:var(--color-code-fn)">$1</span>=<span style="color:var(--color-code-string)">$2</span>`)
    .replace(/(&lt;!--[\s\S]*?--&gt;)/g, `<span style="color:var(--color-code-comment)">$1</span>`);
}

function highlightCss(code: string) {
  let safe = escapeHtml(code);
  safe = safe.replace(/(\/\*[\s\S]*?\*\/)/g, `<span style="color:var(--color-code-comment)">$1</span>`);
  safe = safe.replace(/([.#]?[a-zA-Z_-][\w-]*)\s*(\{)/g, `<span style="color:var(--color-code-keyword);font-weight:600">$1</span> $2`);
  safe = safe.replace(/([a-z-]+)(\s*:)/g, `<span style="color:var(--color-code-fn)">$1</span>$2`);
  safe = safe.replace(/(:\s*)([^;{}\n]+)/g, (_, a, b) => `${a}<span style="color:var(--color-code-string)">${b}</span>`);
  return safe;
}
...
export function CodeBlock({ code, lang = "js", title, showLineNumbers = true }: Props) {
  ...
  return (
    <div className="my-5 overflow-hidden rounded-xl border-2 border-border bg-[var(--color-code-bg)] text-[13px] shadow-[6px_6px_0_0_var(--color-border)]">
