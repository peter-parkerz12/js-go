  ...
  head: () => ({
    meta: [
      { title: "JS:GO — Learn JavaScript from Zero to Pro" },
      {
        name: "description",
        content:
          "Premium docs + bento learning for HTML, CSS, and JavaScript. Beginner to advanced in 60–90 days.",
      },
    ],
  }),
});
      ...
      <section className="grid gap-6 md:grid-cols-12">
        <div className="md:col-span-7">
          <div className="mb-4 flex flex-wrap items-center gap-2">
            <span className="chip">
              <span className="h-1.5 w-1.5 rounded-full bg-[var(--color-success)]" /> {totalLessons}{" "}
              lessons
            </span>
          ...
          <p className="mt-5 max-w-xl text-lg text-muted-foreground">
            A premium documentation platform that teaches HTML, CSS, and modern JavaScript so well
            you will be ready for React or Next.js — without ever feeling lost.
          </p>
...
learn(you).then(ship); // → React-ready`}
      ...
      <section className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {TRACKS.map((t) => (
          ...
          >
            <div
              className="mb-3 inline-flex items-center gap-2 font-mono text-xs font-bold uppercase tracking-wider"
              style={{ color: t.color }}
            >
      ...
      <section className="mt-12">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:auto-rows-[140px] md:grid-cols-4">
          <BentoCard
            span="xl"
            title="0 → Pro Syllabus"
            description="Every JS topic, ordered for the fastest path from beginner to job-ready."
          ...
          >
            <div className="mt-auto flex flex-wrap gap-1">
              {["Foundations", "Functions", "Async", "DOM", "OOP", "FP", "Modern JS"].map((p) => (
                <span
                  key={p}
                  className="rounded-full border-2 border-border bg-background/30 px-2 py-0.5 text-[10px] font-semibold"
                >
                  {p}
                </span>
              ))}
          ...
          <BentoCard
            title="Live Practice"
            description="Playground with instant preview."
            icon={<Code2 className="h-5 w-5" />}
            to="/practice"
          />
          <BentoCard
            title="Search"
            description="Cmd/Ctrl + K, instant results."
            icon={<Search className="h-5 w-5" />}
            to="/bookmarks"
          />
          <BentoCard
            title="Roadmap"
            description="60–90 day plan from zero to React-ready."
          ...
          <BentoCard
            title="Projects"
            description="Real builds you can ship."
            icon={<Dumbbell className="h-5 w-5" />}
            to="/projects"
          />
          <BentoCard
            title="Tips & Discipline"
            description="Deep work, recall, anti-tutorial-hell."
            icon={<Sparkles className="h-5 w-5" />}
            to="/tips"
          />
          <BentoCard
            title="Progress"
            description="Track lessons. Bookmark anything."
            icon={<Trophy className="h-5 w-5" />}
            to="/progress"
          />
          <BentoCard
            title="Productivity"
            description="Habits that compound."
            icon={<Zap className="h-5 w-5" />}
            to="/productivity"
          />
          <BentoCard
            title="20% for React"
            description="The JS that matters most for React."
            icon={<Target className="h-5 w-5" />}
            to="/learn/javascript/twenty-percent-for-react"
            span="wide"
            accent
          />
        </div>
        ...
        <ol className="grid gap-3 sm:grid-cols-2 md:grid-cols-4">
          {[
            { n: "01", title: "HTML & CSS", body: "Structure, layout, responsive design." },
            {
              ...
              body: "Variables, types, functions, scope.",
            },
